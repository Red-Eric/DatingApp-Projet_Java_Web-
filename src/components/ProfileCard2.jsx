...
const ProfileCard2 = ({ user }) => {
    ...
    return (
        <div className="text-center mb-8 md:mb-0 bg-white rounded-2xl">
            <img onClick={viewProfile} src={user?.image} alt="Profile Picture" className="rounded-full w-48 h-48 mx-auto mb-4 border-4 border-indigo-800 dark:border-blue-900 transition-transform duration-300 mt-4" />
            <h1 className="text-2xl font-bold text-indigo-800 dark:text-gray-800 mb-2">{user?.name} {user?.fname}</h1>
            <p className="text-gray-600 dark:text-gray-700">Je suis {user?.sexe === "H" ? "un Homme" : "une Femme"}</p>

            <div className="flex justify-center gap-3 mb-2">
                <button onClick={likePPL} type="button" className={`text-blue-700 border border-blue-700 focus:ring-4 focus:outline-none font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center dark:border-blue-500 dark:text-blue-500 dark:focus:ring-blue-800 ${isLiked ? "bg-blue-700 text-white" : "hover:bg-blue-700 hover:text-white"}`}>
                ...
                <button onClick={disLikePPL} type="button" className={`text-red-700 border border-red-700 focus:ring-4 focus:outline-none font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center dark:border-red-500 dark:text-red-500 dark:focus:ring-red-800 ${isHated ? "bg-red-700 text-white" : "hover:bg-red-700 hover:text-white"}`}>
                    <svg className="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 18">
