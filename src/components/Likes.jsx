    ...
    return (
        <div>
            <div className="bg-white ml-7 rounded-2xl pl-4 h-10 w-[35%] flex justify-around mb-5">
                <h1 className="font-medium text-2xl">Vos J'aimes</h1>
            </div>
...
const LikesPage = () => {
    ...
    useEffect(() => {
        ...
        getUserFromId(parseInt(sessionStorage.getItem("id")))
            .then(data => setVisitorList(data.likes))
    }, [])
    ...
    return (
        <div className="mt-4 ml-5 mr-5">
            {
                visitorList.length > 0 && all.length > 0 ? showAll(visitorList, all) : <h1 className="font-medium text-2xl">Vous avez 0 J'aime(s)</h1>
            }
        </div>
    )
}
